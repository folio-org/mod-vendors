...
public class VendorTest {
  ...
  public void tests(TestContext context) {
    ...
    try {
      ...
      logger.info("--- mod-storage-test: Creating vendor ... ");
      ...
      logger.info("--- mod-orders-storage-test: Verifying only 1 vendor was created ... ");
      getData("vendor").then().log().ifValidationFails()
        .statusCode(200)
        .body("total_records", equalTo(1));

      logger.info("--- mod-orders-storage-test: Fetching vendor with ID: "+ dataSampleId);
      getDataById("vendor", dataSampleId).then().log().ifValidationFails()
        .statusCode(200)
        .body("id", equalTo(dataSampleId));

      logger.info("--- mod-orders-storage-test: Editing vendor with ID: "+ dataSampleId);
      ...
      logger.info("--- mod-orders-storages-test: Deleting vendor with ID ... ");
