...
public class VendorAddressTest {
  ...
  public void tests(TestContext context) {
    ...
    try {
      ...
      logger.info("--- mod-vendors-test: Creating vendor address ... ");
      ...
      logger.info("--- mod-vendors-test: Verifying only 1 vendor address was created ... ");
      getData("vendor_address").then().log().ifValidationFails()
        .statusCode(200)
        .body("total_records", equalTo(1));

      logger.info("--- mod-vendors-test: Fetching vendor address with ID: "+ dataSampleId);
      getDataById("vendor_address", dataSampleId).then().log().ifValidationFails()
        .statusCode(200)
        .body("id", equalTo(dataSampleId));

      logger.info("--- mod-vendors-test: Editing vendor address with ID: "+ dataSampleId);
      ...
      logger.info("--- mod-vendors-test: Deleting vendor address with ID ... ");
