...
public class VendorAddressTest {
  ...
  public void tests(TestContext context) {
    ...
    try {
      ...
      logger.info("--- mod-storage-test: Creating vendor address ... ");
      String dataSample = getFile("vendorAddress.sample");
      Response response = postData("vendor_address", dataSample);
      response.then().log().ifValidationFails()
        .statusCode(201)
        .body("language", equalTo("en"));
      String dataSampleId = response.then().extract().path("id");

      logger.info("--- mod-orders-storage-test: Verifying only 1 vendor address was created ... ");
      getData("vendor_address").then().log().ifValidationFails()
        .statusCode(200)
        .body("total_records", equalTo(1));

      logger.info("--- mod-orders-storage-test: Fetching vendor address with ID: "+ dataSampleId);
      getDataById("vendor_address", dataSampleId).then().log().ifValidationFails()
        .statusCode(200)
        .body("id", equalTo(dataSampleId));

      logger.info("--- mod-orders-storage-test: Editing vendor address with ID: "+ dataSampleId);
      JSONObject catJSON = new JSONObject(dataSample);
      catJSON.put("id", dataSampleId);
      catJSON.put("language", "Gift");
      response = putData("vendor_address", dataSampleId, catJSON.toString());
      response.then().log().ifValidationFails()
        .statusCode(204);

      logger.info("--- mod-orders-storage-test: Fetching vendor address with ID: "+ dataSampleId);
      getDataById("vendor_address", dataSampleId).then()
        .statusCode(200).log().ifValidationFails()
        .body("language", equalTo("Gift"));

      logger.info("--- mod-orders-storages-test: Deleting vendor address with ID ... ");
      deleteData("vendor_address", dataSampleId).then().log().ifValidationFails()
        .statusCode(204);
