...
public class VendorTypeTest {
  ...
  private void verifyCollection() {
    ...
    getData("/vendor-storage/vendor_types").then()
      .log().all()
      .statusCode(200)
  ...
  public void tests(TestContext context) {
    ...
    try {
      ...
      Response response = postData("/vendor-storage/vendor_types", dataSample);
      ...
      getData("/vendor-storage/vendor_types").then().log().ifValidationFails()
        .statusCode(200)
        .body("total_records", equalTo(1));

      logger.info("--- mod-vendors-test: Fetching vendor type with ID: "+ dataSampleId);
      getDataById("/vendor-storage/vendor_types", dataSampleId).then().log().ifValidationFails()
        .statusCode(200)
      ...
      response = putData("/vendor-storage/vendor_types", dataSampleId, catJSON.toString());
      response.then().log().ifValidationFails()
        .statusCode(204);

      logger.info("--- mod-vendors-test: Fetching vendor type with ID: "+ dataSampleId);
      getDataById("/vendor-storage/vendor_types", dataSampleId).then()
        .statusCode(200).log().ifValidationFails()
        .body("value", equalTo("Gift"));

      logger.info("--- mod-vendors-test: Deleting vendor type with ID ... ");
      deleteData("/vendor-storage/vendor_types", dataSampleId).then().log().ifValidationFails()
        .statusCode(204);
