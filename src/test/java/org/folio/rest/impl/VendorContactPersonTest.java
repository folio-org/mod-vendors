...
public class VendorContactPersonTest {
  ...
  private void verifyCollection() {
    ...
    getData("vendor_contact_person").then()
      .log().all()
      .statusCode(200)
  ...
  public void tests(TestContext context) {
    ...
    try {
      ...
      logger.info("--- mod-storage-test: Creating vendor contact person ... ");
      String dataSample = getFile("vendorContactPerson.sample");
      Response response = postData("vendor_contact_person", dataSample);
      response.then().log().ifValidationFails()
        .statusCode(201)
        .body("language", equalTo("en-us"));
      String dataSampleId = response.then().extract().path("id");

      logger.info("--- mod-orders-storage-test: Verifying only 1 vendor contact person was created ... ");
      getData("vendor_contact_person").then().log().ifValidationFails()
        .statusCode(200)
        .body("total_records", equalTo(1));

      logger.info("--- mod-orders-storage-test: Fetching vendor contact person with ID: "+ dataSampleId);
      getDataById("vendor_contact_person", dataSampleId).then().log().ifValidationFails()
        .statusCode(200)
        .body("id", equalTo(dataSampleId));

      logger.info("--- mod-orders-storage-test: Editing vendor contact person with ID: "+ dataSampleId);
      JSONObject catJSON = new JSONObject(dataSample);
      catJSON.put("id", dataSampleId);
      catJSON.put("language", "Gift");
      response = putData("vendor_contact_person", dataSampleId, catJSON.toString());
      response.then().log().ifValidationFails()
        .statusCode(204);

      logger.info("--- mod-orders-storage-test: Fetching vendor contact person with ID: "+ dataSampleId);
      getDataById("vendor_contact_person", dataSampleId).then()
        .statusCode(200).log().ifValidationFails()
        .body("language", equalTo("Gift"));

      logger.info("--- mod-orders-storages-test: Deleting vendor contact person with ID ... ");
      deleteData("vendor_contact_person", dataSampleId).then().log().ifValidationFails()
        .statusCode(204);
