...
public class InterfaceTest {
  ...
  public void tests(TestContext context) {
    ...
    try {
      ...
      logger.info("--- mod-storage-test: Creating interface ... ");
      ...
      logger.info("--- mod-orders-storage-test: Verifying only 1 interface was created ... ");
      getData("interface").then().log().ifValidationFails()
        .statusCode(200)
        .body("total_records", equalTo(1));

      logger.info("--- mod-orders-storage-test: Fetching interface with ID: "+ dataSampleId);
      getDataById("interface", dataSampleId).then().log().ifValidationFails()
        .statusCode(200)
        .body("id", equalTo(dataSampleId));

      logger.info("--- mod-orders-storage-test: Editing interface with ID: "+ dataSampleId);
      ...
      logger.info("--- mod-orders-storages-test: Deleting interface with ID ... ");
