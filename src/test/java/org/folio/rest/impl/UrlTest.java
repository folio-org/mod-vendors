...
public class UrlTest {
  ...
  public void tests(TestContext context) {
    ...
    try {
      ...
      logger.info("--- mod-storage-test: Creating url ... ");
      ...
      logger.info("--- mod-orders-storage-test: Verifying only 1 url was created ... ");
      getData("url").then().log().ifValidationFails()
        .statusCode(200)
        .body("total_records", equalTo(1));

      logger.info("--- mod-orders-storage-test: Fetching url with ID: "+ dataSampleId);
      getDataById("url", dataSampleId).then().log().ifValidationFails()
        .statusCode(200)
        .body("id", equalTo(dataSampleId));

      logger.info("--- mod-orders-storage-test: Editing url with ID: "+ dataSampleId);
      ...
      logger.info("--- mod-orders-storages-test: Deleting url with ID ... ");
